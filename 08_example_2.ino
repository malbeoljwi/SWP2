...
void loop() {
  ...
  if ((distance == 0.0) || (distance > _DIST_MAX)) {
      ...
      digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (distance == 200.0) {    // In desired Range
      analogWrite(PIN_LED, 0);       // LED ON    
  } else if (distance > _DIST_MIN && distance < 200.0 ){
      int LED_Strength = 510 - distance * 255/100;
      analogWrite(PIN_LED, LED_Strength);
  } else {
      int LED_Strength = distance * 255/100 - 510;
      analogWrite(PIN_LED, LED_Strength);
  }
